...
```
___
Предположим, что мы решили построить функцию, которая будет суммировать первые `n` натуральных чисел и выводить результат. Что произойдёт при вызове этой функции?
...
- Функция вернёт сумму первых `n` натуральных чисел ~~Нет, параметр функции `uint` никогда не может быть меньше нуля. Функция уйдёт в бесконечную рекурсию.~~
- Функция вернёт сумму первых `n-1` натуральных чисел ~~Нет, параметр функции `uint` никогда не может быть меньше нуля. Функция уйдёт в бесконечную рекурсию.~~
- Функция ничего не вернёт и уйдёт в бесконечную рекурсию ~~Правильный ответ~~
___
...
___
Какой тип у данной функции?

```go
func Say (a string, b *int, c byte) (x, y string) {
    // .... 
}
```

- `func(string, byte, *int) (string, string)` ~~Нет, присмотритесь к аргументам.~~
- `func(string, *int, byte) (string)` ~~Нет, присмотритесь к аргументам.~~
- `func(string, *int, byte) (string, string)` ~~Верно!~~
___
...
___
Отметьте правильные свойства замыканий

- Замыкание захватывает значение внешних переменных на момент вызова. Оно не влияет на их дальнейшие значения ~~Увы, это не свойство замыкания.~~
- Замыкание захватывает переменную в момент создания замыкания ~~Верно!~~
- Замыкание может изменять значение внешней переменной ~~Верно!~~
- Замыкание обязательно должно изменить значение после каждого вызова ~~Увы, это не свойство замыкания.~~
___
