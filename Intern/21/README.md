...
- `[1 2]` ~~Изменится только локальная копия `s` в методе `Add`.~~
- `[0 0]` ~~Изменится только локальная копия `s` в методе `Add`.~~
- `[]` ~~Правильно!~~
- Ошибка компиляции ~~Изменится только локальная копия `s` в методе `Add`.~~
___
...
1. Функция-поле может быть динамически переопределена во время работы программы. Это позволяет использовать, например, функции из других пакетов.
1. Функция-поле может быть пустой. Тогда её вызов создаст панику.
...
- `func Type::String(id int) string {}` ~~Чувствуется лёгкий привкус С++, но в Go это происходит не так.~~
- `func String(t Type) (id int) string {}` ~~Не в том порядке.~~
- `func (t Type) String (id int) string {}` ~~Верно!~~
___
...
- Ошибка. Программа упадёт, потому что нельзя присваивать получателю по значению константный тип ~~Нет. Присваивать значение можно, но присвоение будет работать только для локальной копии.~~
- `0` ~~Правильный ответ~~
- `5` ~~Присвоение получателю по значению не приводит к изменению самой переменной.~~
___
