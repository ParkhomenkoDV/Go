...
- Первым ~~Неверно. Принято возвращать последним.~~
- Любым ~~Неверно. Принято возвращать последним.~~
- Последним ~~Верно!~~
___
Если требуется сформировать ошибку на лету, добавив в неё какую-нибудь информацию, то как это можно сделать?

- `errors.New("Error is %v", params)` ~~`errors.New()` не принимает форматированную строку с параметрами, только одну строку с сообщением.~~
- `fmt.Errorf("Error is %v", params)` ~~Верно!~~
- `errors.NewError("Error is %v", params)` ~~Такой конструкции нет.~~
___
