...
- `switch v2 := v.(type) { ... }` ~~Если тип не будет найден среди `case`, код просто не выполнится.~~
- `if i, ok := v.(int); ok { ... }` ~~Если тип отличается, ok = false и код не выполнится.~~
- `i := v.(int)` ~~Опасно — если `v` не `int`, то будет паника.~~
- `i, _ := v.(int)` ~~Если `v` не `int`, то `i` будет равен 0.~~
___
Допустим, есть переменная `var u uint64 = 123`. Функция принимает только `uint32`. Как привести переменную `u` к нужному типу?

- `u.(uint32)` ~~Нет, формально типы `uint32` и `uint64` разные. Нужно использовать конструкцию `uint32(u)`, чтобы сделать копию переменной `u`.~~
- `uint32(u)` ~~Да, делаем копию переменной `u`.~~
___
...
- Проверка типов осуществляется на этапе выполнения, это снижает производительность ~~Верно!~~
- Удобно, что можно создавать функции, принимающие любой тип, однако такие функции не так уж часто нужны ~~Тоже правильный ответ~~
- Код становится чище и понятнее ~~Нет, наоборот. Код засоряется многочисленными приведениями типов. А IDE становится труднее вам помогать.~~
- Пустые интерфейсы позволяют сравнивать любые переменные между собой ~~Нет, сравнение интерфейсных переменных может привести или к неправильным результатам, или к панике.~~
___
