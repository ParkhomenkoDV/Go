...
___
Запустите код из последнего примера на своём компьютере. Как вы думаете, будет ли у вас отличаться значение переменной p и почему?

- Будет отличаться. ~~Верно!~~
- Не будет отличаться. Адреса переменных всегда фиксированные.
___
...
p = GetPersonWithLastVisited(p) // переприсваивание
```
...
```
- 5 ~~Неправильно. Рассчитайте ещё раз.~~
- 12 ~~Неправильно. Рассчитайте ещё раз.~~
- 19 ~~Верно!~~
- `Panic error` ~~Неправильно. Поводов для паники нет.~~
___
...
Для доступа к значению переменной, переданной в функцию, используется оператор `*`.
Таким образом при каждом вызове `f` мы будем увеличивать значение переданной переменной.
...
```
___
Выберите правильный способ создания указателя на переменную `а`:
- `p := *a` ~~Оператор `*` получает значение, на которое указывает переменная `а`. Для создания указателя используется `&`.~~
- `p := &a` ~~Верно!~~
- `p := new(a)` ~~Функция `new()` создаёт указатель на основе переданного ей типа. Ей нельзя передать переменную.~~
___
Что делает унарный оператор `*`, применённый к указателю?

- Оператор возвращает значение, на которое «смотрит» указатель. Если его изменить, это не повлияет на переменную ~~Нет, оператор возвращает переменную, на которую «смотрит» указатель.~~
- Оператор возвращает переменную, на которую «смотрит» указатель. Её можно изменить ~~Верно!~~
- Позволяет изменить указатель ~~Указатель применяется через присваивание ему нового значения.~~
___
