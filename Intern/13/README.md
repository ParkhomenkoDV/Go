...
___
Выберите правильный пример кода работы со структурой типа `Person`: 
```go
type Person struct {
    Name        string
    Email       string
...
- `man := Person{"Alex", "alex@yandex.ru"}` ~~Неправильно, так как указаны не все поля структуры.~~
- `man := Person{Name: Alex, Email: alex@yandex.ru}` ~~Неправильно. Строки `Alex` и `alex@yandex.ru` должны быть в кавычках.~~
- 
```
man := Person{
    Name: "Alex", 
...
``` 
~~Неправильно. Не хватает запятой в конце последней строки.~~
- `man := Person{Name: "Alex", Email: "alex@yandex.ru"}` ~~Верно!~~
___
