...
```
___
Соедините вопросы и примеры описания переменных.

- Переменная имеет тип `float64` (`v := 7.01`)
- Значение переменной равно `0` (`var v int`)
- Выражение `v + 1000` выдаст ошибку компиляции (`var v uint = 100)
- Значение переменной равно `-1` (`var v = 5*3 - 16`)
___
...
___
Описаны следующие константы и переменные:
```go
const (
    i         = 10
    f         = 1.5
...
Отметьте выражения, которые не будут выдавать ошибки компиляции.

- `i + f` ~~Правильно!~~
- `i + i64` ~~Правильно!~~
- `f + i64` ~~Ошибка: f (untyped float constant 1.5) truncated to int64.~~
- `i64 + v` ~~Ошибка: invalid operation: i64 + v (mismatched types int64 and int).~~
- `i + v` ~~Правильно!~~
- `f + v` ~~Ошибка: f (untyped float constant 1.5) truncated to int.~~
___
