...
В примере для пакета `calc` создана отдельная директория. Но если не предполагается добавлять в модуль больше одного пакета, можно писать код прямо в директории с файлом `go.mod`.
...
___
Информация о модуле хранится в файле:

- `go.mod` ~~Верно!~~
- `go.sum`
- `main.go`
___
Модуль можно разместить
