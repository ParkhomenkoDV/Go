...
___
Какой командой можно запустить тесты?

- `gotests run`
~~Нет, прочитайте урок ещё раз.~~
- `go test .`
~~Верно!~~
- `go run tests`
~~Нет, прочитайте урок ещё раз.~~
___
Каким методом можно пропустить выполнение теста?

- `go skip test`
~~Такой команды нет.~~
- `t.Fatal()`
~~Это остановит выполнение тестов.~~
- `t.Skip()`
~~Верно!~~
___
...
- [assert](https://pkg.go.dev/github.com/stretchr/testify/assert) — пакет, в котором собрано множество удобных функций с говорящими названиями: вроде `assert.Equal` или `assert.Nil`. Предназначен для использования внутри обычных Go-тестов вида `func TestXxx(t *testing.T)` (про необычные поговорим, когда доберёмся до [suite](https://pkg.go.dev/github.com/stretchr/testify/suite)).
...
```
___
Напишите тест для функции `EstimateValue` в стиле [table-driven](https://github.com/golang/go/wiki/TableDrivenTests).
