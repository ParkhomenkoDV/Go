...
1. Подбирайте пакету уникальное имя в рамках репозитория. В противном случае при импорте нескольких пакетов с одинаковыми именами придётся использовать алиасы.
1. Не пишите в имени пакета общие слова: `util`, `base`, `tools`, `lib`, `common`. Обычно эти имена ничего не рассказывают о реальном назначении пакета и только засоряют пространства имён. Помните, что имя пакета будет использоваться в коде.
1. Не используйте множественное число. Но есть исключения: пакеты `strings`, `bytes`, `errors` в стандартной библиотеке Go названы так, чтобы избежать конфликта с типами. Иногда использовать множественное число допустимо — например, если нужно показать, что пакет `handlers` содержит несколько обработчиков.
...
1. Затем компилятор компилирует пакет `main` и собирает основное приложение, а далее в процессе выполнения программы произойдёт следующее: 
...
___
Допускается ли циклический импорт пакетов?

- Да, допускается ~~Нет, не допускается.~~
- Допускается только для пакетов с пустым импортом ~~Пустой импорт тут ни при чём, а циклический импорт не допускается.~~
- Не допускается ~~Верно!~~
___
Отметьте компоненты, которые будут экспортироваться из пакета.

- `func MakeGood(){}` ~~Верно!~~
- `var public int` ~~Название переменной не имеет значения, в отличие от значения регистра первой буквы.~~
- `const letter = "A"` ~~Первая буква имени в нижнем регистре значит, не экспортируется.~~
- `type Local struct{}` ~~Верно!~~
___
