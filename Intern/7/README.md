...
___
Посмотрите на пример кода и отметьте верные утверждения.
...
- `Result` — глобальная переменная ~~`Result` — локальная переменная, так как определена внутри функции.~~
- Константа `Limit` и переменная `Mode` — экспортируемые ~~Правильно!~~
- `counter` — глобальная экспортируемая переменная ~~`counter` — глобальная, но не экспортируемая переменная.~~
- Функцию `mode()` нельзя вызвать из другого пакета ~~Правильно! Это неэкспортируемая функция.~~
___
Отметьте корректные примеры кода для определения констант и глобальных переменных.


- `var i = 10, j = "hello"` ~~Ошибка `syntax error: unexpected = after top level declaration`.~~
- `var rate, name = 7, "John"` ~~Правильно!~~
- `const VER string = "v1.1.1"` ~~Правильно!~~
- `var initial := 8` ~~`:=` неприменимо для глобальных переменных.~~
- `const height uint8 = iota + 1` ~~Правильно!~~
- `const Weight int` ~~Не указано значение. Ошибка `missing init expr for Weight`.~~
___
