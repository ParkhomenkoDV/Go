...
Итак, вы настроили рабочее окружение и скомпилировали первую программу, пришло время погрузиться в детали.

Вначале рассмотрим понятие типизации и базовые типы данных в Go. Затем расскажем, как объявлять переменные и константы и производить над ними простые операции.
...
___
Может ли тип переменной в Go меняться на стадии исполнения программы?

- Нет, тип переменной всегда должен быть известен на стадии компиляции и при выполнении меняться не может ~~Верно!~~
- Переменная может принимать тип на стадии выполнения, исходя из контекста применения ~~В Go типы всех переменных определяются на стадии компиляции.~~
- Некоторые переменные необязательно должны быть типизированы ~~Все переменные в Go должны быть типизированы.~~
___
Выберите верное утверждение.

- Тип переменной в Go всегда нужно объявлять явно ~~В некоторых случаях тип может вывести компилятор без явной декларации.~~
- Тип объявлять не требуется, он будет выведен на стадии компиляции или исполнения ~~Тип переменной должен быть определён на стадии компиляции. При этом компилятор не всегда способен вывести тип самостоятельно, без явной декларации, — тогда он выводит информативные ошибки типизации.~~
- Во многих случаях, например после операции инициализации переменной, компилятор способен вывести тип из контекста. Если контекста недостаточно, тип нужно декларировать явно. ~~Правильно!~~
___

# Дополнительные материалы
- [Хабр. Ликбез по типизации в языках программирования](https://habr.com/ru/post/161205/)
- [CoderNet. Что такое типизация: введение в программирование для новичков](https://codernet.ru/articles/drugoe/chto_takoe_tipizacziya_vvedenie_v_programmirovanie_dlya_novichkov/)
